...
as(phenoData(exampleSet),"data.frame")[c(1:10),1]
@
...
<<Iorio.RankMerging>>=
MergingSet <- Iorio.RankMerging(exampleSet,distance="Spearman")
show(MergingSet)
@
...
<<ScoreGSEA>>=
ds <- ScoreGSEA(MergingSet,250,DisMeasurement="avg")
ds[1:5,1:5]
@
...
<<SignatureDistance>>=
dds <- SignatureDistance(exampleSet,MergingMethod="Iorio", distance="Spearman", 
ScoringMethod="GSEA", SignatureLength=250, DisMeasurement="avg")
ds[1:5,1:5]
@
...
<<Iorio.RankMerging>>=
IorioMergingSet <- Iorio.RankMerging(exampleSet,distance="Spearman")
...
<<Pihur.RankMerging>>=
PihurMergingSet <- Pihur.RankMerging(exampleSet[1:15])
...
<<ScoreGSEA>>=
ds <- ScoreGSEA(IorioMergingSet,250,"avg")
ds[1:5,1:5]
@

<<ScorePGSEA>>=
ds <- ScorePGSEA(IorioMergingSet,250,"avg")
ds[1:5,1:5]
@
